...
function submissionRecord(item, type) {
  ...
  const title = item.title || item.original_filename || '未命名投稿'
  const detail = item.author
    ? `${item.author} · ${item.category || '未分类'}`
    : `${structure?.profile === 'long' ? '长篇' : '短篇'}结构 · ${Number(structure?.file_count || 0)} 个文件`
  ...
  return node('article', { class: `submission-record${item.status === 'rejected' ? ' needs-attention' : ''}` }, [
    ...
    node('div', { class: 'submission-progress', 'aria-label': `投稿进度：${uploadStatusLabel(item.status)}` }, stageLabels.map((label, index) =>
...
async function loadSubmissionPage(requestedRecordPage = 1) {
  ...
  const [deconstructions, novels, categoryData, notificationData] = await Promise.all([
  ...
  novelForm.addEventListener('submit', async event => {
    ...
    try {
      ...
      feedback.textContent = result.message; await loadSubmissionPage(1)
    } catch (error) { feedback.textContent = error.message }
    finally { submit.disabled = false }
  })

  const allRecords = [
    ...(novels.items || []).map(item => ({ item, type: 'novel' })),
    ...(deconstructions.items || []).map(item => ({ item, type: 'deconstruction' }))
  ].sort((left, right) => new Date(right.item.created_at || 0) - new Date(left.item.created_at || 0))
  const recordPageCount = Math.max(1, Math.ceil(allRecords.length / SUBMISSION_RECORDS_PER_PAGE))
  const recordPage = Math.min(Math.max(1, Number(requestedRecordPage) || 1), recordPageCount)
  const start = (recordPage - 1) * SUBMISSION_RECORDS_PER_PAGE
  const statusCounts = allRecords.reduce((counts, { item }) => {
    if (item.status === 'completed') counts.completed += 1
    else if (item.status === 'approved') counts.approved += 1
    else if (item.status !== 'rejected') counts.processing += 1
    return counts
  }, { processing: 0, approved: 0, completed: 0 })
  const actionRequiredCount = Number(notificationData.action_required_count || 0)
  const records = node('div', { class: 'submission-records', role: 'list' }, allRecords
    .slice(start, start + SUBMISSION_RECORDS_PER_PAGE)
    .map(({ item, type }) => submissionRecord(item, type)))
  if (!records.childElementCount) records.append(node('p', { class: 'page-subtitle', text: '还没有投稿记录。' }))
  const recordPagination = node('nav', { class: 'notification-pagination submission-pagination', 'aria-label': '投稿记录分页' }, [
    node('button', { class: 'notification-page-button', type: 'button', text: '上一页', disabled: recordPage <= 1 ? '' : null, onclick: async () => { await loadSubmissionPage(recordPage - 1) } }),
    node('span', { class: 'notification-page-status', role: 'status', 'aria-live': 'polite', text: `第 ${recordPage} / ${recordPageCount} 页` }),
    node('button', { class: 'notification-page-button', type: 'button', text: '下一页', disabled: recordPage >= recordPageCount ? '' : null, onclick: async () => { await loadSubmissionPage(recordPage + 1) } })
  ])
  app.replaceChildren(node('div', { class: 'account-page' }, [
    ...
    node('section', { class: 'submission-history-panel', 'aria-labelledby': 'submission-history-title' }, [
      node('header', { class: 'submission-history-header' }, [
        node('div', {}, [node('h2', { id: 'submission-history-title', text: '审核与入库记录' }), node('p', { text: `共 ${allRecords.length} 条记录，每页 ${SUBMISSION_RECORDS_PER_PAGE} 条` })]),
        node('a', { class: 'submission-history-link', href: '#/account/notifications', text: '查看审核消息' })
      ]),
      node('div', { class: 'submission-history-summary', 'aria-label': '投稿状态概览' }, [
        node('span', {}, [node('strong', { text: String(statusCounts.processing) }), node('small', { text: '审核中' })]),
        node('span', {}, [node('strong', { text: String(statusCounts.approved) }), node('small', { text: '待入库' })]),
        node('span', {}, [node('strong', { text: String(statusCounts.completed) }), node('small', { text: '已入库' })]),
        node('a', { class: 'submission-summary-action', href: '#/account/notifications' }, [node('strong', { text: String(actionRequiredCount) }), node('small', { text: '未读需处理' })])
      ]),
      records,
      ...(allRecords.length ? [recordPagination] : [])
    ])
  ]))
}
...
async function loadAccountPage() {
  ...
  const uploadButton = node('button', { class: 'primary-button', type: 'submit', text: '上传文件' })
  ...
  uploadForm.addEventListener('submit', async event => {
    ...
    uploadMessage.textContent = '正在上传文件…'
