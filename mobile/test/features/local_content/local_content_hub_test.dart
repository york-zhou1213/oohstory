...
void main() {
  testWidgets('profile tools expose local reading without adding a fifth tab', (
    tester,
  ) async {
    ...
    tester
        .widget<OohPrimaryNavigationBar>(find.byType(OohPrimaryNavigationBar))
        .onDestinationSelected(3);
    await tester.pump(const Duration(seconds: 1));
    await tester.pump();

    final entry = find.text('本地阅读');
  ...
  testWidgets('landing remains usable at phone, tablet and desktop widths', (
    ...
    for (final size in const <Size>[
      ...
      expect(
        find.textContaining('Kindle：未压缩 / PalmDOC / HUFF'),
        findsOneWidget,
      );
      expect(find.textContaining('CBR：RAR4/5 压缩与存储'), findsOneWidget);
