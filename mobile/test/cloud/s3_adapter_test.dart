...
<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  ...
  test('S3 virtual-hosted mode keeps the bucket out of the path', () async {
    final scope = CredentialScope('s3-virtual-host');
    final credentials = MemoryCredentialStore(<String, String>{
      scope.key('access_key'): 'key',
      scope.key('secret_key'): 'secret',
    });
    final transport = FixtureTransport((request, _) async {
      expect(request.uri.host, 'fixture-bucket.s3.us-east-1.amazonaws.com');
      expect(request.uri.path, anyOf('', '/'));
    ...
    final adapter = S3CloudAdapter(
      endpoint: Uri.parse('https://s3.us-east-1.amazonaws.com'),
      bucket: 'fixture-bucket',
      root: 'OOHStory',
      region: 'us-east-1',
      pathStyle: false,
  ...
  test('S3 large writes use multipart and complete conditionally', () async {
    final scope = CredentialScope('s3-multipart');
    final credentials = MemoryCredentialStore(<String, String>{
      scope.key('access_key'): 'key',
      scope.key('secret_key'): 'secret',
    });
    const partSize = 5 * 1024 * 1024;
    final uploadedPartSizes = <int>[];
    var completed = false;
    final transport = FixtureTransport((request, body) async {
      if (request.method == 'POST' &&
          request.uri.queryParameters.containsKey('uploads')) {
        expect(body, isEmpty);
        return CloudHttpResponse.bytes(
          statusCode: 200,
          body: utf8.encode(
      ...
      if (request.method == 'PUT') {
        uploadedPartSizes.add(body.length);
        final number = request.uri.queryParameters['partNumber'];
        expect(request.uri.queryParameters['uploadId'], 'upload-fixture');
        return CloudHttpResponse.bytes(
          statusCode: 200,
          headers: <String, String>{'etag': '"part-$number"'},
        );
      }
      if (request.method == 'POST') {
        expect(request.uri.queryParameters['uploadId'], 'upload-fixture');
        expect(request.headers['if-none-match'], '*');
        final xml = utf8.decode(body);
      ...
      if (request.method == 'HEAD') {
        return CloudHttpResponse.bytes(
          statusCode: 200,
          headers: const <String, String>{'etag': '"complete"'},
        );
      }
      fail('Unexpected multipart request: ${request.method} ${request.uri}');
    });
    final adapter = S3CloudAdapter(
      endpoint: Uri.parse('https://s3.example.test'),
      bucket: 'fixture-bucket',
    ...
    final payload = List<int>.filled(partSize + 3, 7);
    final result = await adapter.write(
      'books/large.azw3',
    ...
    expect(uploadedPartSizes, <int>[partSize, 3]);
    expect(result.etag, '"complete"');
  });

  test(
    'S3 multipart failure attempts an abort and preserves the error',
    () async {
      final scope = CredentialScope('s3-multipart-abort');
      final credentials = MemoryCredentialStore(<String, String>{
        scope.key('access_key'): 'key',
        scope.key('secret_key'): 'secret',
      });
      ...
      final transport = FixtureTransport((request, _) async {
        if (request.method == 'POST' &&
          ...
          return CloudHttpResponse.bytes(
            statusCode: 200,
            body: utf8.encode('<UploadId>abort-fixture</UploadId>'),
          );
        }
        if (request.method == 'PUT') {
        ...
        if (request.method == 'DELETE') {
          expect(request.uri.queryParameters['uploadId'], 'abort-fixture');
          aborted = true;
          return CloudHttpResponse.bytes(statusCode: 204);
        }
        fail('Unexpected abort request: ${request.method} ${request.uri}');
      });
      final adapter = S3CloudAdapter(
        endpoint: Uri.parse('https://s3.example.test'),
        bucket: 'fixture-bucket',
      ...
      await expectLater(
        adapter.write(
          'books/fails.azw3',
          Stream<List<int>>.value(List<int>.filled(partSize, 1)),
        ),
        throwsA(
          isA<CoreException>().having(
            (error) => error.code,
