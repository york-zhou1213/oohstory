...
void main() {
  ...
  test(
    'WebDAV maps failing 207 members and rejects cross-origin hrefs',
    () async {
      final scope = CredentialScope('webdav-multistatus');
      final credentials = MemoryCredentialStore(<String, String>{
        scope.key('username'): 'user',
        scope.key('password'): 'password',
      });
      ...
      final transport = FixtureTransport((_, _) async {
        final response = crossOrigin
            ? '<d:response><d:href>https://other.example.test/dav/OOHStory/book.azw3</d:href>'
                  '<d:status>HTTP/1.1 200 OK</d:status></d:response>'
            : '<d:response><d:href>/dav/OOHStory</d:href>'
                  '<d:status>HTTP/1.1 403 Forbidden</d:status></d:response>';
        return CloudHttpResponse.bytes(
          statusCode: 207,
          body: utf8.encode(_multistatus(<String>[response])),
        );
      });
      final adapter = WebDavCloudAdapter(
        endpoint: Uri.parse('https://webdav.example.test/dav'),
