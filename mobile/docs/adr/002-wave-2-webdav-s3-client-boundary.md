...
4. 云目录第一阶段提供分页浏览、进入目录、刷新、打开受支持电子书、条件新增上传和带
   ...
   原生端的瞬时上传/删除失败写入 AES-256-GCM 加密的文件队列；主密钥保存在系统安全
   存储，队列按 provider/account 分区，并为用户提供待处理数量和显式重试。
...
- 第一阶段直接做后台双向文件镜像：拒绝；原生端已提供文件级持久化、可观察队列，但
  自动后台双向镜像仍需要平台调度、功耗策略和真实弱网证据。
...
- 仍需真实 WebDAV/S3 sandbox、移动/桌面安全存储、弱网、10k 条目和大文件 multipart
  证据后才能默认开启。
- 原生端持久队列已采用加密文件负载与小型索引，不把大文件 Base64 塞入偏好设置；Web
  端不声明持久离线队列。后台文件镜像仍不在本纵切冒充完成。
