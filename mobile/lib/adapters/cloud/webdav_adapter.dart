...
<d:propfind xmlns:d="DAV:"><d:prop><d:resourcetype/><d:getetag/></d:prop></d:propfind>''';
    ...
    try {
      ...
      for (final providerResponse in xmlElements(document, 'response')) {
        if (_successfulResponse(providerResponse)) {
          entries.add(_entryFromResponse(providerResponse));
          continue;
        }
        final status = _responseStatus(providerResponse);
        if (status != null) throw cloudStatusError(status);
      }
      return entries;
    } on CoreException {
      rethrow;
  ...
  int? _responseStatus(String response) {
    for (final status in xmlElements(response, 'status')) {
      final match = RegExp(r'\s([45]\d\d)(?:\s|$)').firstMatch(status);
