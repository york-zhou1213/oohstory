...
class _LocalReaderScreenState extends State<LocalReaderScreen> {
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          trailing: PopupMenuButton<_ReaderAnnotationAction>(
                            tooltip: '管理批注',
                            onSelected: (action) {
                              switch (action) {
                                case _ReaderAnnotationAction.attach:
                                  _attachFileFromReader(item, context);
                                case _ReaderAnnotationAction.attachments:
                                  _manageAttachmentsFromReader(item, context);
                                case _ReaderAnnotationAction.delete:
                                  _deleteAnnotationFromReader(item, context);
                              }
                            },
                            itemBuilder: (context) => [
                              if (!kIsWeb)
                                const PopupMenuItem(
                                  value: _ReaderAnnotationAction.attach,
                                  child: Text('添加附件'),
                                ),
                              if (attachments.isNotEmpty)
                                PopupMenuItem(
                                  value: _ReaderAnnotationAction.attachments,
                                  child: Text('管理附件 (${attachments.length})'),
                                ),
                              const PopupMenuItem(
                                value: _ReaderAnnotationAction.delete,
                                child: Text('删除批注'),
                              ),
                            ],
                          ),
  ...
  Future<void> _attachFileFromReader(
    OfflineAnnotation annotation,
  ...
  Future<void> _manageAttachmentsFromReader(
    OfflineAnnotation annotation,
    BuildContext sheetContext,
  ) async {
    final changed = await showAnnotationAttachmentsDialog(
  ...
  Future<void> _deleteAnnotationFromReader(
    OfflineAnnotation annotation,
    BuildContext sheetContext,
  ) async {
    try {
...
enum _ReaderAnnotationAction { attach, attachments, delete }
