...
--
-- Run this only after deploy/mysql/init.sql. MySQL generates all three passwords;
-- capture the client output in a root-only file, install the three values in the
...
CREATE USER IF NOT EXISTS 'oohstory_library_reader'@'127.0.0.1'
    IDENTIFIED BY RANDOM PASSWORD;
CREATE USER IF NOT EXISTS 'oohstory_public_reader'@'127.0.0.1'
    IDENTIFIED BY RANDOM PASSWORD;
...
GRANT 'oohstory_public_reader_role'@'%'
    TO 'oohstory_public_reader'@'127.0.0.1';

SET DEFAULT ROLE 'oohstory_library_writer_role'@'%'
    TO 'oohstory_library_writer'@'127.0.0.1';
SET DEFAULT ROLE 'oohstory_library_reader_role'@'%'
    TO 'oohstory_library_reader'@'127.0.0.1';
SET DEFAULT ROLE 'oohstory_public_reader_role'@'%'
    TO 'oohstory_public_reader'@'127.0.0.1';
