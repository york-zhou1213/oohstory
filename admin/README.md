...
Version 0.10.0 is an authenticated FastAPI operations backend for OOHStory. It reads the local Reader API, durable MySQL catalog and shared library path, with an optional disposable Redis cache. It inspects bounded host/systemd state and allows only explicit actions through compiled allowlists. Shared-library writes are performed only by a root-owned structured-action helper; the unprivileged web process keeps a read-only catalog account. The admin includes responsive catalog, per-volume cover, publication, pipeline, audit and recovery-oriented operations views.
...
`init.sql` creates the database, all 22 current schema revisions, migration
...
three `127.0.0.1` accounts with MySQL-generated random passwords: pipeline
writer、Admin read-only、Reader public-metrics. Immediately install those values
in their separate password files, then securely remove the root-only capture
file. There are no default credentials or seed production records in Git.
...
- `OOHSTORY_ADMIN_READER_URL` (validated internal HTTP only; loopback by default)
- `OOHSTORY_ADMIN_READER_ALLOWED_HOSTS` (explicit opt-in for an isolated container service name)
...
- `OOHSTORY_ADMIN_SCRIPT_STORE_HELPER_PATH`, `OOHSTORY_ADMIN_MANAGED_SCRIPT_ROOT`
...
For a dedicated `oohstory-admin` user, install `ops/oohstory-admin-systemctl`, `ops/oohstory-admin-script-store`, and `ops/oohstory-admin-library-action` as root-owned mode `0755` under `/usr/local/libexec/`. Keep `ops/oohstory-admin-library-action-runner` root-owned mode `0755` under `/opt/oohstory-admin/ops/`. Install `ops/oohstory-admin.sudoers` as root-owned mode `0440` only after validating it with `visudo -cf`. The service example enables helper mode. Do not make any helper, action runner, managed script, or library environment file writable by the service user.
...
- Pipeline: status and confirmed start/restart/stop/enable/disable actions for reader, authorized sync, cover generation/sync, light/derived indexes, deconstruction sync, and the crawler browser. Each card reports the unit file, working directory, actual runtime script/executable path, and fixed OOHStory-managed path. Seven allowlisted Python entrypoints can be viewed, syntax-checked, concurrency-checked, backed up, and atomically saved; timers and shared entrypoints are represented honestly.
...
python3 -m py_compile ops/oohstory-admin-script-store
