...
def test_bootstrap_is_fresh_only_and_least_privilege() -> None:
    initializer = (MYSQL_ROOT / "init.sql").read_text(encoding="utf-8")
    assert "CREATE TEMPORARY TABLE `_oohstory_fresh_schema_guard`" in initializer
    assert "DROP TEMPORARY TABLE `_oohstory_fresh_schema_guard`" in initializer
    assert "CREATE PROCEDURE" not in initializer
    ...
    assert "GRANT INSERT, UPDATE ON `oohstory_library`.book_public_metrics" in initializer
    assert (
        "GRANT SELECT, INSERT, UPDATE ON "
        "`oohstory_library`.book_public_metric_visitors"
        in initializer
    )
    assert (
