...
class ReadingProgress {
  ...
  ReadingProgress({required this.chapterId, required this.within, required this.timestamp});
...
class ReadingProgressService {
  ...
  Map<String, ReadingProgress> _loadAll() {
    ...
    return map.map((k, v) => MapEntry(k, ReadingProgress.fromJson(v as Map<String, dynamic>)));
  }

  void _saveAll(Map<String, ReadingProgress> data) {
    _prefs.setString(_key, jsonEncode(data.map((k, v) => MapEntry(k, v.toJson()))));
  }
  ...
  void save(String bookId, String chapterId, double within) {
    ...
    if (all.length > 100) {
      final sorted = all.entries.toList()..sort((a, b) => b.value.timestamp.compareTo(a.value.timestamp));
