...
class TtsService {
  ...
  void _buildSmartPlan(List<String> paragraphs, int startParagraph) {
    ...
    for (int i = startParagraph; i < paragraphs.length; i++) {
      ...
      if (!isDialogue) {
        _plan.add(
          _TtsItem(url: _buildItemUrl(line, narratorVoice), paraIdx: i),
        );
        continue;
      }
  ...
  Future<void> play({int fromIndex = 0}) async {
    ...
    await _playAt(_currentIndex, generation);
  }

  Future<void> _playAt(int index, int generation) async {
    if (!active || generation != _generation || index >= _plan.length) {
    ...
    try {
      ...
      _stateSub = _player.playerStateStream.listen((state) {
        if (state.processingState == ProcessingState.completed &&
          ...
          _stateSub?.cancel();
          _playAt(index + 1, generation);
        }
      });
    } catch (_) {
      if (active && generation == _generation) _playAt(index + 1, generation);
    }
  }
